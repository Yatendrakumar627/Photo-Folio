      ...
      <div className={styles.albumContent} >
        <div className={styles.albumHeader} >
          ...
          <div className={styles.AlbumButton}
              ...
              id={albumFormStatus ? "red" : "blue"}>
            {albumFormStatus ? "cancel" : "Add album"}
          </div>
        </div>
        <div className={styles.albumContentContainer}>
          {albumData.map((album,id) => (
            <div key={id} 
              ...
              onClick={()=>{handleSwitchRender(album)}} > 
            <img 
              src='image-thumb.png' width="80%" alt='img-thumbnail' />
              <p>{album.album}</p>
          </div>
          ))}
          {albumData.length === 0 ? <p> No items!, Add new album </p> : null}
        </div>
      </div>
    </section>
  )
