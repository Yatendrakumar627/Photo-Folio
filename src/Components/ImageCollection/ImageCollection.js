...
const Photos = ({albumId}) => {
  ...
  return (
    <div className={styles.photoSection}>
      {photos.map((photo, id) => (
        <div 
            className={styles.photoContainer} key={id}
            onClick={()=> handleImageView(id)}  >
          <div className={styles.buttons} >
              <div><img 
                    src="delete.png" alt="dlt" 
        ...
        {photos.length === 0 ? 
        <p> No item!, Add new images </p> : <p className={styles.imageCount}> Total Images ({photos.length}) </p> }
          {imageViewStatus ? 
            <ImageViewer
            imageUrl={photos[currentImageIndex].url}
            handleImageViewStatus={handleImageView}
